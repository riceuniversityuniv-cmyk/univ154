...
- **Resolved**: user clarified the "information below" WAS the boss-style questions themselves (math check + portfolio-value ask) — not separate content that failed to paste. Nothing further needed there.

### 2026-08-26 — Module 1: made "gross income" visually unmissable on the Results hero card
Follow-up same day. User pointed out the word "gross" existed but was easy to miss — it was only in 13px, 50%-opacity caption text below a 56px bold headline number, and the label directly above the number ("Required Annual Retirement Income") didn't say gross at all. Three changes to the same hero card, no math changed:
- `incomeLabel` now reads "Required **Gross** Annual/Household Retirement Income" (was missing the word entirely).
- Added a high-contrast gold pill badge ("GROSS, BEFORE TAX") directly beside the big number itself, not just in a caption below it.
- Brightened the caption line from 50% to 75% opacity, bolded "Gross" in gold, and added "— see **Take-Home** below for the after-tax number" so the gross/net distinction is stated outright, not just implied by two separate numbers on the page.
- Verified live via the same temporary `/__qa/week-0` route + Playwright pattern as the entry above (Texas, single filer, defaults) — screenshotted the hero card, confirmed all three cues render together. Route removed before finishing (`git diff src/App.jsx` clean). `npm run build` and `npx eslint` clean.
...
### 2026-09-23 — 2026 state tax tables refreshed; site-wide math audit against the Excel workbook
User forwarded a ChatGPT critique of the Income Taxes page ($90k, NY, NYC): NY rates were the 2025 ones, the 5.25% bracket displayed as "5.3%", and state/NYC tax uses the federal taxable base instead of a NY base. All three confirmed. Rather than patch NY only, every state table was diffed against Tax Foundation's 2026 table (raw HTML, parsed by script, not summarized) — 23 states changed: AR CA GA ID IN KY ME MD MN MS MO MT NE NY NC OH OK OR RI SC UT VT WI (mostly 2026 rate cuts and inflation-indexed thresholds; MD added 6.25%/6.5% top brackets). Deliberate exceptions and sources:
- **GA 4.99%, UT 4.45%, SC 1.99% / 5.21% over $30,000** come from newer primary sources (GA governor's release for HB 463, SCDOR page, Utah SB 60) than the Tax Foundation snapshot (5.19% / 4.5% / old 3%-6% ladder). SC also decoupled from the federal deduction (SCIAD), which the tool doesn't model.
- **NJ left at 5.525%** (Tax Foundation rounds to 5.53%). **WA left at 0%** (capital-gains-only tax, no wage tax). Iowa stays flat 3.8%.
- Cross-check: `Week0CourseIntro.jsx`'s independently authored table already held the 2026 values for the 20 states that had no later law change, so the two data sets agree everywhere. Week 0 itself was aligned for GA/UT/SC and two typos (DE 5.5% → 5.55%, NJ 5.53% → 5.525%).
- **Still a simplification, not changed:** state and NYC tax are computed on the FEDERAL taxable income (no NY $8,000 standard deduction etc.). Doing this per-state needs a std-deduction/exemption dataset (Tax Foundation's table has one); ask before building.
- Display: state/NYC bracket rates and SS/Medicare rates now show 2 decimals (`formatPercent(..., { decimals: 2 })`) so 5.15% no longer shows as 5.2% and Medicare 1.45% no longer as 1.5%.

Audit method (Excel workbook `Final Master Copy - Web Based Application (Fixed).xlsx` cached values as ground truth): Week 3 loops (PMT, min payment, payoff) vs closed form; Week 2 NPER/PMT vs Excel; Week 5/11 mortgage vs `Week 11.1`; Week 6 401(k)/IRA vs `Week 5.2`/`5.5` (three scenarios each, matched to the cent); Week 7 insurance vs `Week 9 - Insurance`; Week 9 vs `Week 6 - Markets & Investing` (FV, gross withdrawals, taxes at 15% all match); Week 12 rendered headlessly (react-dom/server) vs `Week 7 B - Results` (portfolio income $110,058, balance $10,606,156 match). Fixes found: Week 5 bi-weekly final payment now `MIN(balance + interest, payment)` like Excel (previously forced a spurious extra period; displayed 17.3 yrs/208 mo/$242,410 unchanged for the default case); Week 2 savings no longer returns NaN at a 0% earning rate; Week 6 static "$23,500" / "$7,000" limit text now reads the live Assumptions limits. Deliberately left alone: Excel's "Total Costs" adds annual taxes/insurance to the monthly payment (Week 5 mirrors it); Week 12 default Notes/IOUs is $0 vs Excel's $1,000; Week 9 uses bracketed LTCG instead of Excel's flat 15%; Week 6 IRA blurb still says "$150k income" (approximate phase-out wording, unverified for 2026).
Deploy: migration `20260923010000_update_state_brackets_2026.sql` applied to the live DB on 2026-09-23 via the Management API (one-time PAT, not stored) and verified — all 54 live bracket sets equal the code defaults, 12 scalars and 49 RMD rows already matched. The temporary DEV-only preview override in `AssumptionsContext.jsx` was removed before commit. Open decisions left for the user: RMD start age 73 vs 75 (SECURE 2.0: 75 for anyone born 1960+; Excel Week 5.2 already says 75), and per-state standard deductions (NY $8,000 etc.).

### 2026-09-23 — RMD start age changed 73 → 75
User confirmed the tool's audience is college students (born ~2000s), so SECURE 2.0's age 75 (anyone born 1960+) applies, not 73. Excel's Week 5.2 sheet already said 75 while its Assumptions sheet said 73, and `Week6Retirement.jsx` originally hardcoded 75 before the assumptions consolidation. Changed the `rmd_start_age` scalar (code default + migration `20260923020000_rmd_start_age_75.sql`, applied live and verified with a one-time PAT, not stored), replaced three hardcoded `|| 73` input limits in Week 6 with the setting, and annotated `week12FormulaSpec.md` (its sample rows still show the workbook's age-73 numbers). Effect: Week 12 forced withdrawals start two years later; only visible when the retirement projection runs past 75.

### 2026-09-23 — State/NYC tax now use the state's own standard deduction + personal exemption
Previously state and NYC tax ran on the FEDERAL taxable income (a documented simplification inherited from the Excel workbook). User asked to fix it after the ChatGPT critique ($98,200 pre-tax, NY + NYC: NY tax now $4,753.55 and NYC $3,371.32 on a $90,200 state base, vs $4,275.65 / $3,057.37 before). Added `stateDeductions` (single filer, 51 entries: `{ std, personal }`, with `std: 'federal'` for the 8 states that just follow the federal deduction: CO ID IA MO MT NM ND DC) sourced from Tax Foundation's 2026 table. New engine functions `calculateStateDeductionAmount` / `calculateStateTaxableIncome`; `calculateFullTax` returns `stateTaxableIncome` and feeds it to both state and NYC brackets. Week 1 State Tax, Week 4 (new "State Taxable Income" row + state bracket table) and Week 12 (working-year and retirement-year state tax, deduction indexed to the retirement year) switched to the state base; the Summary keeps showing federal taxable income (Excel C9/F9).
